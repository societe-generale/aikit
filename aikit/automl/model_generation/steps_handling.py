...
def get_needed_steps(df_information, type_of_problem):
    ...
    is_regression = type_of_problem == enums.TypeOfProblem.REGRESSION

    has_text = any((enums.TypeOfVariables.TEXT in v["TypeOfVariable"] for v in df_information.values()))
    has_category = any((enums.TypeOfVariables.CAT in v["TypeOfVariable"] for v in df_information.values()))
    ...
    if is_regression:
        needed_steps.append({"step": enums.StepCategories.TargetTransformer, "optional": True})

    if has_text:
        needed_steps.append({"step": enums.StepCategories.TextPreprocessing, "optional": True})
        needed_steps.append({"step": enums.StepCategories.TextEncoder, "optional": False})
        needed_steps.append({"step": enums.StepCategories.TextDimensionReduction, "optional": True})

    if has_category:
        needed_steps.append({"step": enums.StepCategories.CategoryEncoder, "optional": False})

    if has_missing_value:
        needed_steps.append({"step": enums.StepCategories.MissingValueImputer, "optional": False})

    needed_steps.append({"step": enums.StepCategories.Scaling, "optional": True})
    needed_steps.append({"step": enums.StepCategories.DimensionReduction, "optional": True})
    needed_steps.append({"step": enums.StepCategories.FeatureExtraction, "optional": True})
    needed_steps.append({"step": enums.StepCategories.FeatureSelection, "optional": True})

    needed_steps.append({"step": enums.StepCategories.Model, "optional": False})
...
def create_var_type_from_steps(models_by_steps):
    ...
    for step_name, model_name in models_by_steps.items():
        ...
        if model_name[0] is None:
            var_type = enums.StepCategories.get_type_of_variable(step_name)
        else:
...
def modify_var_type_none_to_default(var_type_by_steps, default_var_type):
    ...
    for (step_name, model_name), var_type in var_type_by_steps.items():
        ...
        if enums.StepCategories.is_composition_step(step_name):
            continue
...
def modify_var_type_alldefault_to_none(var_type_by_steps, default_var_type):
    ...
    if all_default:
        for step_name, model_name in var_type_by_steps.keys():
            if not enums.StepCategories.is_composition_step(step_name):
