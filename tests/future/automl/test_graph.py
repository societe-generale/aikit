...
from aikit.future.enums import VariableType
from aikit.future.graph import convert_graph_to_code
from aikit.future.graph._check import is_composition_model, assert_model_graph_structure
from aikit.future.graph._convert import _create_name_mapping, _find_first_composition_node
from aikit.future.graph._factory import _must_include_selector, create_graphical_representation
...
def test_is_composition_model():
    ...
    for model in MODEL_REGISTRY.informations.keys():
...
def test_create_graphical_representation():
    steps = OrderedDict(
        [
            (("TextPreprocessing", ("TextPreprocessing", "CountVectorizerWrapper")), VariableType.TEXT),
            (("DimensionReduction", ("DimensionReduction", "TruncatedSVDWrapper")), VariableType.TEXT),
            (("CategoryEncoder", ("CategoryEncoder", "NumericalEncoder")), VariableType.CAT),
            (("CategoryImputer", ("CategoryImputer", "CatImputer")), VariableType.CAT),
            (("MissingValueImputer", ("MissingValueImputer", "NumImputer")), VariableType.NUM),
            (("FeatureExtraction", ("FeatureExtraction", "PolynomialExtractor")), VariableType.NUM),
            (("Scaling", ("Scaling", "StandardScaler")), (VariableType.CAT, VariableType.NUM)),
            (
                ("FeatureSelection", ("FeatureSelection", "FeaturesSelectorClassifier")),
                (VariableType.CAT, VariableType.NUM, VariableType.TEXT),
            ),
            (
...
def test_convert_graph_to_code():
    ...
    # 1 composition step above a pipeline
    ...
    # Remark: blending specification is missing, BUT it is enough to test the function
    all_models_params = {
    ...
    with pytest.raises(ValueError):
        model_json_code = convert_graph_to_code(Graph, all_models_params)
        # Unsupported for now : more than one terminal node

    model_json_code = convert_graph_to_code(Graph, all_models_params, check_structure=False)
    ...
    # 1 composition with several nodes bellow
    Graph = nx.DiGraph()
    # TODO : make a stacking with several models below
    ...
    # Remark : the Stacker is missing the blender, that I can't enter into the graph.
    assert expected_json_code == model_json_code

    # With a node above, and a blender below
    Graph = nx.DiGraph()
    Graph.add_edge(
...
def test_assert_model_graph_structure():
    ...
    # This edge makes it impossible : RF can't have two parents, if one is a composition
